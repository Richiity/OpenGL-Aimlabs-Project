...
vec3 Laser2() {
	vec3 p1 = Laser1();
	return p1+5*normalize((FingerTip(Right)-p1)); // JB: changed
}
...
void MakeScene() {
	...
	ReadMesh(rightHand, "HandRight.obj");
...
void Keyboard(int key, bool press, bool shift, bool control) {
	if (press && key == ' ' && targeted) {
		// hits.push_back(target);	// JB: changed
		startBillboardShot = true;
	}
}
