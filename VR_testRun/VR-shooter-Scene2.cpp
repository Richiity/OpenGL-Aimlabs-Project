...
// meshes
Mesh		bench, ground, head, leftHand, rightHand, bill2, bill3, pistol, box;
...
//translate (LR / Height/ Depth)
void MakeScene() {
	string objDir("C:/Users/deant/Code/Graphics/Assets/Models/"), imgDir("C:/Users/deant/Code/Graphics/Assets/Images/");
	if (!bench.Read(objDir + "target_sphere.obj", imgDir + "target_sphere.jpeg")) printf("can't read bench\n");
	if (!bill2.Read(objDir + "target_sphere.obj", imgDir + "target_sphere.jpeg")) printf("can't read bench\n");
	if (!bill3.Read(objDir + "target_sphere.obj", imgDir + "target_sphere.jpeg ")) printf("can't read bench\n");
	if (!ground.Read(objDir + "Test_ground.obj", imgDir + "sandtexture.jpg")) printf("can't read floor\n");
	if (!box.Read(objDir + "aimlab_box.obj", imgDir + "rocktexture.jpg")) printf("can't read floor\n");
	if (!head.Read(objDir + "Head.obj")) printf("can't read head\n");
	if (!leftHand.Read(objDir + "Hand.obj") || !rightHand.Read(objDir + "Hand.obj")) printf("can't read hand\n");
	if (!pistol.Read(objDir + "Pistol2.obj", imgDir + "pistol2.png")) printf("cant read pistol\n");
	//boxes start
	bench.toWorld = Scale(.25f) * Translate(0, .7f, 20.0f) * RotateZ(90) * RotateX(0) * RotateY(90);
	bill2.toWorld = Scale(.25f) * Translate(4.7f, .7f, 16.5f) * RotateZ(90) * RotateX(45) * RotateY(90);
	bill3.toWorld = Scale(.25f) * Translate(-6.7f, .7f, 18.7f) * RotateZ(90) * RotateX(-45) * RotateY(90);
	//boxes end
	ground.toWorld = Scale(4) * Translate(0, -.3f, 0);
	box.toWorld = Scale(4) * Translate(0, -.05, 1.1f);
